...
const DelAccountPage = () => {
  ...
  const [selectedArtId, setSelectedArtId] = useState(null); // state to hold selected space_id
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSelectedArtId(artworks.filter(art => art.art_id !== selectedArtId)); // Update the UI
      handleCloseModal();
    } catch (error) {
